<?php
	...
	if($passed_step == 5){
		...
		$captcha_site_key			= isset($_SESSION['captcha_site_key']) ? prepare_input($_SESSION['captcha_site_key']) : '';
		$captcha_secret_key	  = isset($_SESSION['captcha_secret_key']) ? prepare_input($_SESSION['captcha_secret_key']) : '';
	...
	// Mail
		$mail_driver		 			  = isset($_SESSION['mail_driver']) ? prepare_input($_SESSION['mail_driver']) : '';
		$mail_host							= isset($_SESSION['mail_host']) ? prepare_input($_SESSION['mail_host']) : '';
		$mail_port							= isset($_SESSION['mail_port']) ? prepare_input($_SESSION['mail_port']) : '';
		$mail_username					= isset($_SESSION['mail_username']) ? prepare_input($_SESSION['mail_username']) : '';
		$mail_password					= isset($_SESSION['mail_password']) ? prepare_input($_SESSION['mail_password']) : '';
		$mail_encryption    	  = isset($_SESSION['mail_encryption']) ? stripslashes($_SESSION['mail_encryption']) : '';
		...
		if(empty($error_mg)){
			if(EI_MODE == 'demo'){
				if($database_host == 'localhost' && $database_name == 'dship' && $database_username == 'root' && $database_password == ''){
					$completed = true;
				}else{
					...
					if(EI_CHECK_DB_MINIMUM_VERSION && (version_compare($db->GetVersion(), EI_DB_MINIMUM_VERSION, '<'))){
						...
						if($sql_dump != ''){
							if(false == ($db_error = apphp_db_install($sql_dump_file))){
								...
								$config_file = str_replace('<CAPTCHA_SITE_KEY>', $captcha_site_key, $config_file);
								$config_file = str_replace('<CAPTCHA_SECRET_KEY>', $captcha_secret_key, $config_file);
								$config_file = str_replace('<DB_PREFIX>', $database_prefix, $config_file);
								//Mail
								$config_file = str_replace('<MAIL_DRIVER>', $mail_driver, $config_file);
								$config_file = str_replace('<MAIL_HOST>', $mail_host, $config_file);
								$config_file = str_replace('<MAIL_PORT>', $mail_port, $config_file);
								$config_file = str_replace('<MAIL_USERNAME>', $mail_username, $config_file);
								$config_file = str_replace('<MAIL_PASSWORD>', $mail_password, $config_file);
								$config_file = str_replace('<MAIL_ENCRYPTION>', $mail_encryption, $config_file);
								...
								///////////////////////////////////////////////
								if ($install_type == 'create' || $install_type == 'update'){
									$file = 'script.zip';
				...
				<?php if($install_type == 'update'){ ?>
					...
					<tr>
						<td>
							...
							<br /><br />
							<a href="<?php echo '../'.EI_APPLICATION_START_FILE;?>">

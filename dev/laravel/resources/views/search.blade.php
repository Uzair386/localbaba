...
@section('description', 'Search Page | '.$settings->site_name.'')
@section('content')
<!-- =========================
   Product Section
   ============================== -->
   ...
   <div class="container">
      <div class="row">
          ...
          <form method="get" action="{{ url('/search') }}">
            ...
            <div class="side-bar category category-md">
               ...
               <ul class="dropdown-list-menu">
                  ...
                  <li>
                     <a href="{{route('category_page', ['slug'=>$category->slug])}}"><i class="fa fa-angle-double-right" aria-hidden="true"></i> {{$category->name}}</a>
                  </li>
                  @endforeach
               </ul>
            </div>
         ...
         <div class="order-1 order-md-1 col-12 col-md-8 col-lg-9 product-grid">
            <div class="row">
               <div class="col-12">
                  <div class="filter row d-flex justify-content-between align-items-center py-2">
                     ...
                     <div class="col-6 col-md-6 filter-btn-area text-center">
                        {{ __('messages.You_searched_for') }} <b>{{$query}}</b>
                     </div>
                     <div class="col-4 col-md-3 text-right">
                        <select class="form-control cart66-select" id="sel1"  name="product_id" style="font-size: inherit">
                           <option @if(request()->get('order') == null) selected @endif data-url="{{url('/search?query=').request()->get('query')}}">Sort by</option>
                           <option @if(request()->get('order') == 'name') selected @endif data-url="{{url('/search?query=').request()->get('query')}}&order=name">Product Title</option>
                           <option @if(request()->get('order') == 'stock_h') selected @endif data-url="{{url('/search?query=').request()->get('query')}}&order=stock_h">Highest Inventory</option>
                           <option @if(request()->get('order') == 'stock_l') selected @endif data-url="{{url('/search?query=').request()->get('query')}}&order=stock_l">Lowest Inventory</option>
                           <option @if(request()->get('order') == 'price_h') selected @endif data-url="{{url('/search?query=').request()->get('query')}}&order=price_h">Highest Price</option>
                           <option @if(request()->get('order') == 'price_l') selected @endif data-url="{{url('/search?query=').request()->get('query')}}&order=price_l">Lowest Price</option>
                        </select>
                     </div>
                  </div>
               </div>

...
<div class="card">
  ...
  <div class="card-block">
    ...
    <hr> @endif
    <form action="{{route('work.post.update',['id'=>$post->id])}}" method="post" enctype="multipart/form-data">
      {{csrf_field()}}
      <div class="row">
        <label class="col-sm-4 col-lg-2 col-form-label">Page Title</label>
        <div class="col-sm-8 col-lg-10">
          <div class="input-group">
            <input type="text" value="{{$post->title}}" class="form-control fill" required name="title"
              placeholder="Name">
          </div>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-sm-4 col-lg-5">
          <div class="input-group">
            <img src="{{asset($post->image)}}" alt="{{$post->title}}" width="100px" height="100px" />
          </div>
        </div>
        <div class="col-sm-4 col-lg-5">
          <div class="input-group">
            <input id="image" class="form-control fill" name="image" type="file">
          </div>
        </div>
      </div>
      <div class="row">
